...
;  KK= - Kalman gain
function tutorial_ukf,fd,g,t0,xkm1,PPkm1,t1,nstep,z,QQ,RR,PP=PP,ZZ=ZZ,KK=KK
  n=n_elements(xkm1);
  m=n_elements(z);
  ...
  rPPkm1=PPkm1                           
  la_choldc,rPPkm1,/upper; %Matrix square root (upper triangluar, P=A'*A)
  for i=1,n-1 do rPPkm1[0:i-1,i]=0 ;erase the lower triangular
  ...
  ;used. "
  ;Matlab (and IDL with la_choldc(/upper)) produces an upper triangular 
  ;P=A'*A, so it matches the first form
  ;and we need to use rows. This will result in a vector with n nonzero
  ...
  for i=0,n-1 do begin
    ...
    xjkm1[0,*,i+1  ]=xkm1+transpose(rPPkm1[*,i]);   % (7) taking rows of rPPkm1
    ;Negative sigma point
    xjkm1[0,*,i+1+n]=xkm1-transpose(rPPkm1[*,i]);   % (8) taking rows of rPPkm1
  end;
  ...
  Gammak=RR;          %Accumulate innovation covariance (paper Cov(z~fkm1)), start with additive measurement noise
  SSk=dblarr(m,n);    %Accumulate cross covariance      (paper Cov(x~fk,z~fkm1))
  for j=0,2*n do begin
    Gammak=Gammak+WW[j]*(zfjk[0,*,j]-zfk)##transpose(zfjk[0,*,j]-zfk); % (16)
    SSk=SSk+WW[j]*(xfjk[0,*,j]-xfk)##transpose(zfjk[0,*,j]-zfk);       % (17)
  end
  
  ;Data Assimilation
  KK=SSk ## kf_inv(Gammak);
  ;KKk=SSk/Gammak;             % (19)
  ZZ=z-zfk; %Innovation, calculated separately so can be returned
  xk=xfk+KK##ZZ;             % (18)
  PP=PPfk-KK##Gammak##transpose(KK);   % (20) This is not the (1-KH)P that the linear filter uses
  return,xk
end